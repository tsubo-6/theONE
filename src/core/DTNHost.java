...
public class DTNHost implements Comparable<DTNHost> {
	...
	//今まで訪れたことのある目的地を格納する配列
	//private List<Coord> destinationHistory;
	
	//ノードが避難所に到達したことを表すフラグ
	private boolean arrivingNodeFlag = false;
	
	private boolean goShelter = false;
	
	//フラグのフラグ
	private boolean goSwitch=false;
	
	//避難場所に到達したノード数を表す変数
	private static int arrivingNodeCounter;
	...
	public DTNHost(List<MessageListener> msgLs,
		...
		//this.destinationHistory = new ArrayList<Coord>();
	...
	public void move(double timeIncrement) {		
		...
		//一定の時間間隔ごとに移動する距離を計算
		// timeIncrement : 0.1　(設定ファイルの8行目)
		possibleMovement = timeIncrement * speed;
		//次の中継地点までの距離
		distance = this.location.distance(this.destination);
		//移動アルゴリズム
		//possibleMovementがdistance以上である限りループ
		while (possibleMovement >= distance) {
			// node can move past its next destination : ノードは次の目的地を過ぎても移動できる			
			this.location.setLocation(this.destination); // snap to destination : 現在位置を目的地にセットする操作
			possibleMovement -= distance;
			if(goShelter&&!goSwitch) {
				//System.out.println("ノード"+address+"現在のパス:" + path);
				path = null;
				//System.out.println("ノード"+address+"の現在位置(Coord)"+getLocation());
				path = movement.getPath(1,getLocation());
				//System.out.println("ノード"+address+"変更後のパス:"+path);
				goSwitch=true;
				return;
			}
			// 新しいwaipointを設定
			if (!setNextWaypoint()) { // get a new waypoint
				...
				if(goSwitch||(address >= 0 && address <= 71)) {
					...
					arrivingNodeFlag=true;
					return;
				}
				return; // no more waypoints left
			}
			// 目的地までの距離を再計算(Coordクラスの関数)
			distance = this.location.distance(this.destination);
		}
	...
	private boolean setNextWaypoint() {
		if (path == null) {
			// 最後に通過したMapNodeからランダムに選択された目的地までの最短経路を格納
		...
		//destination: x座標とy座標が格納されている
		this.destination = path.getNextWaypoint();
//		System.out.println("次のwaypoint"+this.destination);
		this.speed = path.getSpeed();
		
		//System.out.println("ノード番号"+address+"過去の中継地点と次の中継地点"+destinationHistory);
		if (this.movListeners != null) {
			for (MovementListener l : this.movListeners) {
				//newDestination : 新しい目的地情報を受け取り時、ホストの状態を更新する関数
