...
public abstract class ActiveRouter extends MessageRouter {
	 ...
	 * accepted a message.
	 */
	// このルーターが保持しているすべてのメッセージを
	// ノードが持つ全ての接続に送信しようと試みる
	protected Connection tryAllMessagesToAllConnections(){
		...
		// getMessageCollection() :ルータ(中継ノード)が保持しているめっせーじのコレクション
	...
	public void update() {		
		...
		// メッセージを送信中の接続を確認
		// メッセージの送信が完了しているか(完了していたら接続を閉じる)、接続がダウンしているかを判定
		// sendingConnections:メッセージを送信するために使用している接続
		for (int i=0; i<this.sendingConnections.size(); ) {
			...
			// removeCurrentがTrueの時、接続をリストから削除する
			if (removeCurrent) {
